...
\item Let $\eps > 0$, $C_{\eps} := [-\eps/2, \eps/2]^d$. Note that one obtains a tiling of $\R^d$ by placing a cube $C_{\eps}$ on each point of $\eps\Z^d$. Let $L_{\eps} := \{x \in \eps\Z^d \colon x + C_{\eps} \subset K\}$ and $U_{\eps} := \{x \in \eps \Z^d \colon (x + C_{\eps}) \cap K \neq \emptyset\}$. Then $|L_{\eps}| \leq |K \cap \eps\Z^d| \leq U_{\eps}|$ and $|L_{\eps}| \leq \frac{\vol(K)}{\eps^d} \leq |U_\eps|$.

\item Which of the inequalities in part 1 are tight?

\end{enumerate}

\begin{proof}

We start with the first statement of the first part. Let $x \in L_{\eps}$. Then $x \in \eps\Z^d$ and $x + C_{\eps} \subset K$ by definition of $L_{\eps}$, so especially $x \in \eps\Z^d \cap K$. Secondly, if $x \in \eps\Z^d \cap K$, we have $x \in (x + C_{\eps}) \cap K \neq \emptyset$ and thus $x \in U_{\eps}$. Therefore, we have $|L_{\eps}| \leq |K \cap \eps\Z^d| \leq |U_{\eps}|$.

From the definition of $L_\eps$ and $U_\eps$, we can conclude

\[\bigcup_{x \in L_\eps} x + C_\eps \subset K \subset \bigcup_{x \in U_\eps} x + C_\eps \,,\]

where the union is disjoint in both cases. This yields

\[|L_\eps| \cdot \vol(C_\eps) \leq \vol(K) \leq |U_\eps| \cdot \vol(C_\eps)\]

and thus also

\[ |L_{\eps}| \leq \frac{\vol(K)}{\eps^d} \leq |U_\eps| \,.\]

Each of these inequalities is tight. As a first example consider the case $K := C_\eps$. Then $|U_\eps| = 1 = |K \cap \eps\Z^d|$ and $|U_\eps| = 1 = \frac{\vol(K)}{\eps^d}$. Now let $K_{\delta} := [-(\eps + \delta), (\eps + \delta)]$ for $\delta > 0$ considerably smaller than $\eps$. Then $|K_\delta \cap \eps \Z^d| = 1 = |U_\eps|$ and $\lim_{\delta \rightarrow 0} \frac{\vol(K_\delta)}{\eps^d} = \lim_{\delta \rightarrow 0} \frac{(\eps + \delta)^d}{\eps^d} = 1 = |U_\eps|$.
