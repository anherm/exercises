...
\item Let $(b_1, b_2)$ be a reduced basis of $\Lambda$. Then $b_1$ is a shortest vector of $\Lambda$.
...
For the first statement, consider the usual picture. Consider the lattice hyperplanes parallel to $b_1$ that contain lattice vectors to test whether there is any lattice vector shorter than $b_1$. The only hyper planes that possibly intersect the circuit of radius $\|b_1\|$ around $0$ go through $0$, $b_2$ and $-b_2$. Since $b_1$ belongs to the basis, it is primitive and no vector on the line through $0$ can be shorter than $b_1$. On the line through $b_2$, there is no lattice vector shorter than $b_2$ since $|\mu_{12}| \leq \frac{1}{2}$. For $-b_2$, one argues similarly.
